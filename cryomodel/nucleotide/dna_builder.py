...
def _2bp_read_polyline_pdb(path: Path) -> np.ndarray:
    ...
    if suf in (".mrc", ".map", ".ccp4", ".mtz"):
        raise ValueError(
            f"Centerline file has map extension ({suf!r}): {path}. "
            "Use the polyline .pdb from dnaaxis (e.g. dna_axis.pdb), not the axis density .mrc."
        )
    try:
        text = path.read_text(encoding="utf-8")
    except UnicodeDecodeError as exc:
        raise ValueError(
            f"Centerline path does not look like a text PDB file: {path}. "
            "Use --centerline-pdb pointing to the CA trace .pdb from dnaaxis extract, not a binary map."
        ) from exc
    pts = []
