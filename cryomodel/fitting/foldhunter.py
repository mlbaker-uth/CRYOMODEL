# cryomodel/fitting/foldhunter.py
...
from ..maps.synthetic_from_model import synthetic_density_zyx_on_grid
...
def pdb_to_density_map(
    ...
    """Convert PDB/mmCIF to density on the target grid (same pipeline as ``model2map``).

    Trilinear impulse deposit + Gaussian blur to ``resolution_A`` (FWHM), MRC corner origin.
    ``plddt_min`` None = include all non-hydrogen atoms (no pLDDT filter).
    """
    st = gemmi.read_structure(str(pdb_path))
    return synthetic_density_zyx_on_grid(
        st,
        target_shape,
        target_apix,
        np.asarray(target_origin, dtype=np.float32),
        resolution_A,
        skip_hydrogen=remove_hydrogens,
        plddt_min=plddt_min,
...
def compute_atom_inclusion_score(
    ...
    # Same frame as zonal-refine / ringer: gemmi grid when ``read_map`` populated it.
    n_in_density = 0
    for x, y, z in atoms_xyz:
        p = np.array([x, y, z], dtype=np.float64)
        density_val = sample_density_at_position(density_map, p, density_threshold=0.0)
...
def foldhunter_search(
    ...
    search_preset: str = "full",
    seed_rotation: Optional[np.ndarray] = None,
    rank_by: str = "correlation",
) -> List[FoldHunterResult]:
    """
    Run FoldHunter search with progressive subsampling (FFT cross-correlation).

    ``search_preset``: ``full`` (slow, global), ``fast`` (fewer rotations / coarser),
    ``refine`` (skip global coarse pass; requires ``seed_rotation`` quaternion w,x,y,z).

    ``rank_by``: ``correlation`` (default), ``inclusion`` (atom-in-map score), ``combined``.
    Translation search always uses FFT CC; ranking can emphasize inclusion when desired.

    Probe-from-PDB uses the same trilinear + Gaussian model as ``cryomodel model2map``.
    ``plddt_min`` None = no pLDDT filtering (all heavy atoms).
    """
    preset = (search_preset or "full").strip().lower()
    if preset not in ("full", "fast", "refine"):
        raise ValueError("search_preset must be full|fast|refine")
    rank_mode = (rank_by or "correlation").strip().lower()
    if rank_mode not in ("correlation", "inclusion", "combined"):
        raise ValueError("rank_by must be correlation|inclusion|combined")
    ...
    print(f"  search_preset={preset}, rank_by={rank_mode}, pLDDT filter={'off' if plddt_min is None else f'>={plddt_min}'}")
    ...
    target_apix_coarse = 4.5 if preset == "fast" else 3.5
    max_peaks_coarse_eff = min(max_peaks_coarse, 8) if preset == "fast" else max_peaks_coarse
    medium_angle_range = 18.0 if preset == "fast" else 30.0
    medium_angle_step = 4.0 if preset == "fast" else 3.0
    fine_angle_range = 4.0 if preset == "fast" else 5.0
    medium_max_peaks_cap = 12 if preset == "fast" else 20

    coarse_peaks: List[dict]

    if preset == "refine":
        if seed_rotation is None:
            raise ValueError("search_preset='refine' requires seed_rotation (w,x,y,z quaternion)")
        seed_q = np.asarray(seed_rotation, dtype=np.float32).reshape(4)
        coarse_peaks = [
            {
                "rotation": seed_q,
                "translation_vox": np.zeros(3, dtype=np.float32),
                "correlation": 1.0,
            }
        ]
        print("\n" + "=" * 70)
        print("ROUND 1: SKIPPED (refine preset — using seed rotation)")
        print("=" * 70)
    else:
        # ========================================================================
        ...
        # ========================================================================
        print("\n" + "=" * 70)
        print(f"ROUND 1: COARSE SEARCH (~{target_apix_coarse} Å/voxel)")
        print("=" * 70)
        ...
        coarse_step = coarse_angle_step
        if coarse_step is None:
            coarse_step = compute_optimal_angular_step(max_dim_coarse, actual_apix_coarse, resolution_A)
        if preset == "fast":
            coarse_step = max(float(coarse_step), 12.0)
        print(f"Coarse angular step: {coarse_step:.2f}°")

        n_rot = n_coarse_rotations
        if n_rot is None:
            n_rot = int((360.0 / coarse_step) ** 2.5)
            n_rot = max(500, min(5000, n_rot))
        if preset == "fast":
            n_rot = min(int(n_rot), 280)

        n_rotations = n_rot
        if symmetry is not None:
            n_rotations = max(1, n_rot // symmetry)
            print(f"Using symmetry {symmetry}: searching {360/symmetry:.1f}° rotation space")

        print(f"Sampling {n_rotations} rotations for coarse search...")
        ...
        for i, q in enumerate(coarse_quaternions):
            ...
            for translation_vox_coarse, corr_val in translation_results:
                ...
                coarse_candidates.append(
                    {"rotation": q, "translation_vox": translation_vox_full, "correlation": corr_val}
                )
        ...
        coarse_peaks = find_broad_peaks(
            ...
            min_peak_correlation=0.2,
            max_peaks=max_peaks_coarse_eff,
        )
        print(f"Identified {len(coarse_peaks)} broad peaks from coarse search")
    
    # ========================================================================
    # ROUND 2: MEDIUM SEARCH (2-3 Å/pixel)
    # ========================================================================
    print("\n" + "=" * 70)
    print(f"ROUND 2: MEDIUM SEARCH (2-3 Å/pixel, ±{medium_angle_range:.0f}° around coarse peaks)")
    print("=" * 70)
    ...
    medium_peaks = find_broad_peaks(
        ...
        max_peaks=min(medium_max_peaks_cap, len(coarse_peaks) * 2),
    )
    ...
    # ========================================================================
    print("\n" + "="*70)
    print(f"ROUND 3: FINE SEARCH (original map, ±{fine_angle_range:.0f}°)")
    print("=" * 70)

    fine_step_deg = fine_angle_step
    ...
    def _rank_key(r: FoldHunterResult) -> float:
        pen = r.out_of_bounds_penalty
        if rank_mode == "inclusion":
            return r.atom_inclusion_score - pen
        if rank_mode == "combined":
            return r.correlation + 0.35 * r.atom_inclusion_score - pen
        return r.correlation - pen

    final_results.sort(key=_rank_key, reverse=True)

    print(f"\nFinal results: {len(final_results)} candidates (rank_by={rank_mode})")
    print(f"Top {min(top_n_candidates, len(final_results))} candidates:")
    for i, result in enumerate(final_results[:top_n_candidates], 1):
        rk = _rank_key(result)
        print(
            f"  {i}. Correlation: {result.correlation:.4f}, "
