# cryomodel/validation/ringer_lite.py
...
def get_chi1_atoms(residue: gemmi.Residue) -> Optional[Tuple[str, str, str, str]]:
    ...
    """
    resname = residue.name.strip().upper().split()[0][:3]

    # Standard χ1: N-CA-CB-CG (or equivalent)
    if resname in ['SER', 'CYS', 'THR', 'VAL']:
        # CB-CG is first side-chain bond
        if sole_atom(residue, "N") and sole_atom(residue, "CA") and sole_atom(residue, "CB"):
        ...
        if (
            sole_atom(residue, "N")
            and sole_atom(residue, "CA")
            and sole_atom(residue, "CB")
            and sole_atom(residue, "CG")
        ):
            return ('N', 'CA', 'CB', 'CG')
...
def rotate_sidechain_around_chi1(
    ...
    n = sole_atom(residue, n_name)
    ca = sole_atom(residue, ca_name)
    cb = sole_atom(residue, cb_name)
    cg = sole_atom(residue, cg_name)
...
def sample_density_at_position(
    ...
    *,
    ...
    Maps from :func:`cryomodel.io.mrc.read_map` use gemmi's ``FloatGrid.interpolate_value`` in the
    file's orthogonal Å frame (matches gemmi/ChimeraX). Volumes built without ``grid`` use
    ``origin_xyzA`` + isotropic ``apix`` (tests / legacy).

    If ``density_threshold`` > 0, the returned value is ``max(0, raw - density_threshold)``
    so low continuous background in cryo-EM maps does not act as a positive target.
    """
    px, py, pz = float(position[0]), float(position[1]), float(position[2])
    if map_vol.grid is not None:
        v = float(map_vol.grid.interpolate_value(gemmi.Position(px, py, pz), order=1))
        if density_threshold > 0.0:
            v = max(0.0, v - float(density_threshold))
        return v
    ...
    # Convert to voxel coordinates (legacy orthogonal box)
...
def ringer_scan_residue(
    ...
    n_name, ca_name, cb_name, cg_name = chi1_atoms
    cg_atom = sole_atom(residue, cg_name)
    if not cg_atom:
        return {
            'ringer_Z': 0.0,
            'ringer_peak_deg': 0.0,
