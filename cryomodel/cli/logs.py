...
from ..workflow.activity_log import DOT_DIR, ACTIVITY_FILENAME
...
def _load_project_activity(path: Path) -> list[dict]:
    """Load .cryomodel/activity.jsonl records for workflow-ui runs."""
    if not path.exists():
        return []
    records = []
    with open(path, "r", encoding="utf-8") as handle:
        for line in handle:
            ...
            try:
                ...
                continue
            # Normalize fields to match history schema enough for filters/formatting.
            rec.setdefault("tool", rec.get("card_id") or rec.get("source") or "workflow_ui")
            rec.setdefault("exit_code", rec.get("return_code"))
            records.append(rec)
    return records


def _load_history_with_fallback(base: Path) -> tuple[list[dict], Path]:
    """
    Prefer project activity log first, then fallback to legacy command history.

    Returns (records, selected_path).
    """
    activity_path = base / DOT_DIR / ACTIVITY_FILENAME
    records = _load_project_activity(activity_path)
    if records:
        return records, activity_path
...
def _format_record(record: dict) -> str:
    tool_label = record.get("tool") or record.get("source") or "unknown"
    parts = [
...
def _resolve_output_path(record: dict, base: Path) -> Optional[Path]:
    out = record.get("output_log")
    if not out:
        return None
    p = Path(str(out))
    if p.is_absolute():
        return p
    # activity.jsonl stores project-relative paths like ".cryomodel/runs/<id>.log"
    return base / p
...
def show(
    ...
    records, selected_path = _load_history_with_fallback(base)
    if not records:
        typer.echo(f"No history found in {selected_path}")
        raise typer.Exit(0)
    ...
    if json_output:
        if include_output:
            for record in filtered:
                output_path = _resolve_output_path(record, base)
                if output_path:
