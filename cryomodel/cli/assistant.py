...
def ask(
    ...
    Examples:
        ...
        cryomodel assistant ask "foldhunter failed with low correlation" --tool foldhunter
        cryomodel assistant ask "What does the threshold parameter do?" --tool findligands
    """
...
def suggest(
    ...
    Examples:
        cryomodel assistant suggest "build a model from sequence" --files sequence.fasta,map.mrc
        cryomodel assistant suggest "find ligands" --files model.pdb,map.mrc --generate workflow.yaml
    """
    assistant = CryoModelAssistant()
    ...
    if generate:
        ...
        typer.echo(f"\nRun with: cryomodel workflow run {generate_path}")
    else:
...
def _get_history_parameter_value(param_name: str, tool_name: str, cwd: Path) -> Any:
    history_path = cwd / ".cryomodel_history.jsonl"
...
def troubleshoot(
    ...
    Examples:
        ...
        cryomodel assistant troubleshoot "low correlation" --tool foldhunter
    """
    assistant = CryoModelAssistant()
...
def resolution(
    ...
    Examples:
        ...
        cryomodel assistant resolution 3.5
    """
    assistant = CryoModelAssistant()
...
def diagnose(
    ...
    if history:
        history_path = Path.cwd() / ".cryomodel_history.jsonl"
        recent = []
