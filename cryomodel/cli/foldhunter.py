...
from ..fitting.foldhunter import foldhunter_search, quaternion_to_rotation_matrix

app = typer.Typer(no_args_is_help=True)


def _parse_seed_quaternion(s: Optional[str]) -> Optional[np.ndarray]:
    if s is None or not str(s).strip():
        return None
    parts = [float(x.strip()) for x in str(s).replace(" ", "").split(",") if x.strip()]
    if len(parts) != 4:
        raise typer.BadParameter("Expected four comma-separated floats: w,x,y,z")
...
def search(
    ...
    plddt_threshold: Optional[float] = typer.Option(
        None,
        "--plddt-threshold",
        help="Minimum pLDDT (0–1) to keep atoms; omit to use all non-hydrogen atoms",
    ),
    weight_by_plddt: bool = typer.Option(
        False,
        "--weight-by-plddt",
        help="Weight synthetic probe atoms by pLDDT (only meaningful with --plddt-threshold)",
    ),
    search_preset: str = typer.Option(
        "full",
        "--search-preset",
        help="full (global search), fast (fewer rotations), refine (skip coarse; needs --seed-rotation)",
    ),
    seed_rotation: Optional[str] = typer.Option(
        None,
        "--seed-rotation",
        help="Quaternion w,x,y,z for --search-preset refine (comma-separated)",
    ),
    rank_by: str = typer.Option(
        "correlation",
        "--rank-by",
        help="Final ranking: correlation | inclusion | combined",
    ),
    ...
    """Run FoldHunter: FFT cross-correlation fit of probe (PDB→map uses same model as ``model2map``)."""
    ...
    seed_q = _parse_seed_quaternion(seed_rotation)
    if preset == "refine" and seed_q is None:
        typer.echo("Error: --search-preset refine requires --seed-rotation w,x,y,z", err=True)
        raise typer.Exit(1)

    out_dir.mkdir(parents=True, exist_ok=True)

    typer.echo("Running FoldHunter search...")
    ...
    typer.echo(f"  Search preset: {preset}")
    typer.echo(f"  Rank by: {rank_by}")
    if plddt_threshold is None:
        typer.echo("  pLDDT filter: off (all non-H atoms)")
    else:
        typer.echo(f"  pLDDT threshold: {plddt_threshold}")
    if weight_by_plddt:
        typer.echo("  Weight synthetic map by pLDDT: yes")
    ...
    with open(results_csv, "w", encoding="utf-8") as f:
        f.write(
            "rank,correlation,atom_inclusion_score,n_atoms_in_density,n_atoms_total,out_of_bounds_penalty,"
            "translation_x,translation_y,translation_z,rotation_w,rotation_x,rotation_y,rotation_z\n"
        )
        for i, result in enumerate(results, 1):
            f.write(
                f"{i},{result.correlation:.6f},{result.atom_inclusion_score:.4f},"
