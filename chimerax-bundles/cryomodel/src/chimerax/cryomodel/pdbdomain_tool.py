...
from cryomodel.domains.domain_identifier import (
    identify_domains,
...
from cryomodel.domains.pdbcom import (
    parse_domain_spec,
    compute_domain_coms,
...
class PDBDomainTool(ToolInstance):
    ...
    def _build_ui(self):
        ...
        model_browse.clicked.connect(
            lambda: self._browse_file(
                self.model_path, "PDB/mmCIF (*.pdb *.cif *.mmcif)"
            )
        )
        ...
        domain_browse.clicked.connect(
            lambda: self._browse_file(
                self.domain_path, "Domain files (*.json *.csv *.txt)"
            )
        )
    ...
    def _browse_file(self, line_edit: QLineEdit, filt: str):
        path, _ = QFileDialog.getOpenFileName(
            self.tool_window.ui_area, "Select file", "", filt
        )
        if path:
    ...
    def _run_domains(self):
        model_path = (
            Path(self.model_path.text()).expanduser()
        ...
        chain_id = self.chain_id.text().strip() or (
            structure[0][0].name if len(structure[0]) > 0 else ""
        )
        if not chain_id:
        ...
        if self.auto_identify.isChecked():
            ...
            write_domain_spec(
                out_prefix.with_suffix(".json"), chain_id, ranges_by_domain
            )
            write_domain_csv(
                out_prefix.with_suffix(".csv"), records, ranges_by_domain
            )
            write_domain_pdb(
                structure,
                chain_id,
                ranges_by_domain,
                out_prefix.with_suffix(".pdb"),
            )
            domain_spec = {
                k: {
                    chain_id: [
                ...
                for k, ranges in ranges_by_domain.items()
            }
        else:
            domain_file = (
                Path(self.domain_path.text()).expanduser()
                if self.domain_path.text().strip()
                else None
            )
            if domain_file is None:
                self._log(
                    "Domain file not provided and auto identification is disabled."
                )
                return
        ...
        domain_coms = compute_domain_coms(
            structure, domain_spec, mass_weighted=True, atom_filter="all"
        )
    ...
    def _use_selection_as_plane(self):
        ...
        if len(atoms) < 3:
            self._log(
                "Select at least 3 atoms or residues (CA used) to define a plane."
            )
            return
    ...
    def _analyze_coms(self):
        if self._plane_points is None:
            self._log(
                "No reference plane set. Use 'Use selection as plane' first."
            )
            return
        ...
        out_prefix = Path(self.out_prefix.text()).expanduser()
        com_path = (
            out_prefix.with_name(out_prefix.name + "_com").with_suffix(".pdb")
        )
                ...
                if dist < 1e-6:
                    ...
                    angle = float(
                        np.degrees(
                            np.arccos(
                                np.clip(
                                    abs(np.dot(v / dist, normal)), -1.0, 1.0
                                )
                            )
                        )
                    )
        ...
        out_csv = out_prefix.with_name(
            out_prefix.name + "_com_analysis.csv"
        )
        ...
        self._log(
            f"COM analysis complete: {len(rows)} pairs, {len(plane_rows)} domains."
        )
