...
def parse_residue_tokens(s):
    ...
    for t in s.split(','):
        ...
        if m:
            ...
            tokens.append((chain,int(m.group(2)))); continue
...
def write_mesh_pdb(path, rows, axis_u, rings=24):
    ...
    coords=[]; bvals=[]; occs=[]
    for r in rows:
        c=np.array([r['x'],r['y'],r['z']],float); rad=max(MIN_PORE_RADIUS_A,float(r['radius_A'])); occ=float(r.get('occ_value',0.0))
        if use_local:
...
def main(args=None):
    ...
    p.add_argument("--step",type=float,default=1.0); p.add_argument("--eps",type=float,default=0.25); p.add_argument("--noH",action="store_true"); p.add_argument("--noHet",action="store_true")
    ...
    p.add_argument("--occupancy",choices=["hydro","electro","radii"],default="hydro"); p.add_argument("--hydroscale",choices=["raw","01"],default="raw"); p.add_argument("--electroscale",choices=["raw","01"],default="raw")
    ...
    atoms=load_pdb_atoms(Path(ns.pdb), include_h=not ns.noH, include_hetatm=not ns.noHet)
    ...
    with open(summary_path,'w') as jf:
        json.dump({'pdb':str(Path(ns.pdb).resolve()),'top':ns.top,'bottom':ns.bottom,'centerline':ns.centerline,
                   ...
                   'no_hydrogens':bool(ns.noH), 'ignore_hetatm':bool(ns.noHet)}, jf, indent=2)
...
def run_pyhole(
    ...
    step=1.0, eps=0.25, rings=24, noH=False, noHet=False,
    ...
    atoms=load_pdb_atoms(pdb_path, include_h=not noH, include_hetatm=not noHet)
    ...
    elif occ.startswith("rad"): occ="radii"
    else: occ="hydro"
    ...
    with open(summary_path,'w') as jf:
        json.dump({'pdb':str(pdb_path.resolve()),'top':top,'bottom':bottom,'centerline':centerline,
                   'adaptive':bool(adaptive),'slope_thresh':float(slope_thresh),'max_refine':int(max_refine),
                   'curve_radius_A':float(curve_radius),'curve_iters':int(curve_iters),
                   'step_A':float(step),'eps_A':float(eps),
                   ...
                   'min_radius_A':float(min([r['radius_A'] for r in rows]) if rows else 0.0),
                   'num_samples':len(rows),'occupancy_metric':occ,'hydroscale':hydroscale,'electroscale':electroscale,
                   'no_hydrogens':bool(noH), 'ignore_hetatm':bool(noHet)}, jf, indent=2)
