...
```bash
# 1. Preflight check
cryomodel fitprep check --model model.pdb --map map.mrc --apply

# 2. Find ligands and waters
cryomodel findligands --map map.mrc --model model.pdb --thresh 0.5 \
  --half1 half1.mrc --half2 half2.mrc --ml-model models/ensemble \
  --entry-resolution 2.2

# 3. Predict ligand identities
cryomodel predictligands --ligands-pdb outputs/ligands.pdb \
  --ligand-map outputs/ligands_map.mrc --model model.pdb

# 4. Validate model
cryomodel validate --model model.pdb --map map.mrc \
...
| `pathwalker` | Legacy backbone tracing | `--map`, `--threshold`, `--n-residues`, `--pseudoatom-method` |
| `pathwalker-average` | Average pathwalker PDBs | `--path-files`, `--output-pdb`, `--probabilistic` |
...
### Compare two states
```bash
cryomodel fitcompare compare --model-a stateA.pdb --model-b stateB.pdb \
  --anchors "A:100-160"
```

### Trace DNA centerline
```bash
cryomodel dnaaxis extract --map dna.mrc --threshold 0.25 --guides-pdb dna_markers.pdb
```

### Build poly-AT DNA from centerline (2-bp template)
```bash
cryomodel dnabuild build-2bp --centerline-pdb centerline.pdb \
  --template-2bp-pdb data/DNA-TEMPLATES/2AT-template.pdb --out-pdb dna_2bp.pdb
```

### Analyze pore
```bash
cryomodel pyhole analyze --pdb channel.pdb \
  --top "A:123" --bottom "A:456" --adaptive
```

### Generate loops
```bash
cryomodel loopcloud generate --model model.pdb \
